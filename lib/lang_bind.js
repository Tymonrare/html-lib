...
(function(){
	...
	local.properties = {
		...
		animationInterval : 0.01, //Speed of animation
	...
	local.parseDom = function(dom){
		...
		if(local.properties.saveOriginHTML)
			dom._origin_innerHTML = dom.innerHTML;
	...
	var processValuesChange = function(key, data){
		...
		if(local.val.dynamic.hasOwnProperty(key+"_animated")){
			var animation = function(){
				local.val.dynamic[key+"_animated"] = Math.lerp(local.val.dynamic[key+"_animated"], local.val.dynamic[key], 0.4);
